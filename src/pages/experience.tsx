...
const Experience = () => (
  <main className="flex flex-col items-center justify-center min-h-[80vh] px-4 py-12 md:py-20 animate-fade-in">
    <h1 className="font-playfair text-2xl md:text-4xl text-blue-700 font-bold text-center mb-8">Experience</h1>
    ...
    <div className="space-y-8 w-full max-w-xl mx-auto">
      {experiences.map((exp, i) => (
        <Card key={i} className="border-l-4 border-blue-700 bg-white/90 dark:bg-black/30 rounded-2xl shadow-md p-0">
          <CardHeader className="pb-2">
            <div className="flex flex-col gap-1 md:flex-row md:items-end md:justify-between">
              <CardTitle className="font-playfair text-lg text-blue-700">{exp.role}</CardTitle>
              <span className="text-xs text-blue-700 font-bold">{exp.period}</span>
            </div>
