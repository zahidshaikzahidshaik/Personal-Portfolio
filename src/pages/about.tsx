...
const skills = ["React", "Python", "Spark", "SQL"];
		...
		<Card className="w-full max-w-4xl mx-auto shadow-2xl rounded-3xl border-0 bg-white/80 dark:bg-black/50 py-0 px-0 relative overflow-hidden flex flex-col md:flex-row items-center md:items-start gap-0">
			...
			<div className="flex-1 p-6 md:p-12 flex flex-col gap-4">
				<CardHeader className="p-0 pb-2">
					<CardTitle className="font-playfair text-blue-800 text-3xl md:text-4xl text-left">
						About Me
					</CardTitle>
					<CardDescription className="text-blue-900/80 dark:text-blue-100 text-base md:text-lg text-left mt-1">
						...
						<span className="font-semibold text-blue-800">
							frontend, backend, and data-driven solutions
						</span>
						. Focused on quality code, clean interfaces, and working with
						creative teams.
					</CardDescription>
				</CardHeader>
				{/* Skills badges */}
				<div>
					<h2 className="font-playfair text-xl text-blue-800 mb-2 mt-1">
						Skills
					</h2>
					<ul className="flex flex-wrap gap-2">
						{skills.map((skill) => (
							<li
								key={skill}
								className="bg-blue-100 text-blue-800 px-4 py-1 rounded-full text-sm font-semibold shadow hover:scale-105 transition"
							>
								{skill}
							</li>
						))}
				...
				{/* Education */}
				<div>
					<h2 className="font-playfair text-xl text-blue-800 mb-2">
						Education
					</h2>
					{education.map((edu, i) => (
						<div key={i} className="mb-3">
							...
							{edu.location && (
								<p className="text-blue-700">{edu.location}</p>
							)}
